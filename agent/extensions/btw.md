...
- Thinking content shown in dim italic; the `│` left border is reapplied to every wrapped line (including explicit `\n` and terminal-driven wraps)
- Answer text gets the `│` left border on every wrapped line — long lines that exceed the available width wrap with the border intact, and explicit `\n` newlines each get their own border
