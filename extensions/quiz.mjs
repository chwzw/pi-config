...
const QuizParams = Type.Object({
  ...
  options: Type.Array(OptionSchema, {
    description: "The answer options (2 or more). Options only \u2014 there is no free-text mode. Give each option a stable `value`; you reference the correct one by that value in correctAnswer.",
    minItems: 2
  }),
  ...
  explanation: Type.String({
    description: "REQUIRED. Explanation revealed AFTER the user answers (shown whether they got it right or wrong). Use it to reinforce why the correct answer is correct."
  }),
  shuffle: Type.Optional(
    Type.Boolean({
      description: "Defaults to true: options are randomly reordered before display so the correct answer isn't always in the same position. Set to false only when option order is meaningful (e.g. ordered numeric values, or an 'All/None of the above' option that must stay last)."
    })
  )
});
...
function resolveCorrect(correctAnswer, options) {
  if (correctAnswer === void 0) return { indices: [], error: "correctAnswer is required" };
...
function buildStructuredResult(status, question, mode, answers, correctIndices, correct, explanation, context, message, options, dontKnow, note) {
  return { status, question, context, mode, answers, correctIndices, options, correct, dontKnow, note, explanation, message };
}
function cancelledResult(question, mode, correctIndices, context) {
  const message = "User cancelled the quiz";
  return {
    content: [{ type: "text", text: message }],
    details: buildStructuredResult("cancelled", question, mode, [], correctIndices, void 0, void 0, context, message)
  };
}
function unavailableResult(question, mode, message, correctIndices, context) {
  return {
    content: [{ type: "text", text: message }],
    details: buildStructuredResult("unavailable", question, mode, [], correctIndices, void 0, void 0, context, message)
  };
}
...
function renderFeedback(lines, theme, width, options, selectedIndices, correctIndices, explanation, dontKnow = false, note) {
  ...
  lines.push("");
  if (dontKnow) {
    add(theme.fg("warning", " \xB7 You said: I don't know"));
    const correctStr = correctIndices.map((i) => formatOptionRef(options, i)).join(", ");
...
function pushHeader(lines, theme, width, question, context) {
...
function pushDontKnowRow(lines, theme, width, focused) {
  lines.push("");
...
function pushNoteField(lines, theme, width, editor, focused) {
  lines.push("");
...
async function askSingleChoice(ctx, question, context, options, correctIndices, explanation) {
  ...
  return ctx.ui.custom(
    (tui, theme, _kb, done) => {
      ...
      function response() {
        const note = noteText();
        return dontKnow ? { dontKnow: true, note, answers: [] } : { dontKnow: false, note, answers: chosen ? [chosen] : [] };
      }
...
async function askMultiChoice(ctx, question, context, options, correctIndices, explanation) {
  ...
  const submitItem = { id: "submit", label: "Submit", value: "__submit__", index: -1, isSubmit: true };
  const allItems = [...choiceItems, dontKnowItem, submitItem];
  return ctx.ui.custom(
    (tui, theme, _kb, done) => {
      let optionIndex = 0;
      let phase = "select";
      ...
      const realAnswers = () => sortAnswers(Array.from(selected.values()).filter((a) => a.index !== DONT_KNOW_INDEX));
      function response() {
        const note = noteText();
        return choseDontKnow() ? { dontKnow: true, note, answers: [] } : { dontKnow: false, note, answers: realAnswers() };
      }
      ...
      function render(width) {
        ...
        for (let i = 0; i < allItems.length; i++) {
          ...
          if (item.id === DONT_KNOW_ID) {
            lines.push("");
            const checked2 = selected.has(item.id);
            const label2 = `${checked2 ? "[x]" : "[ ]"} ${item.label}`;
            const styled2 = isFocused ? theme.fg("accent", label2) : theme.fg(checked2 ? "warning" : "dim", label2);
            add(`${prefix}${styled2}`);
            continue;
          }
...
function quiz(pi) {
  pi.registerTool({
    ...
    promptSnippet: "Use the quiz tool to test the user with a graded multiple-choice or multi-select question (required correct answer + required explanation). For non-graded questions, use ask_user_question.",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      try {
        ...
        return unavailableResult(params.question, mode, `quiz ${e.message}`, [], context);
      }
      ...
      return withUILock(async () => {
        const response = mode === "single-select" ? await askSingleChoice(ctx, params.question, context, options, correctIndices, explanation) : await askMultiChoice(ctx, params.question, context, options, correctIndices, explanation);
        if (!response) {
          return cancelledResult(params.question, mode, correctIndices, context);
        }
    ...
    renderResult(result, _options, theme) {
      ...
      const lines = [];
      const displayed = details.options && details.options.length > 0 ? details.options : details.answers.map((a) => ({ index: a.index, label: a.label }));
      ...
      lines.push("");
      const verdict = details.dontKnow ? theme.fg("warning", "I don't know") : details.correct ? theme.fg("success", "Correct!") : theme.fg("error", "Incorrect");
      lines.push(verdict);
      if (details.note) {
